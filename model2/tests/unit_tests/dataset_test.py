...
# ============================================================
def test_label_shape_and_values(label):

    # type
    assert isinstance(label, torch.Tensor), "Label is not torch.Tensor"

    # shape
    assert label.shape == (len(config.CLASS_NAMES),), f"Wrong label shape: {label.shape}"

    # dtype (float32)
    assert label.dtype == torch.float32, f"Wrong label dtype: {label.dtype}"
    ...
    for v in unique_vals:
        assert v in allowed, f"Invalid label value found: {v}"

    print("----- Test of label shape & values passed. -----\n")
...
def test_train_dataset_single_sample(train_dataset):
    ...
    test_label_shape_and_values(label)
